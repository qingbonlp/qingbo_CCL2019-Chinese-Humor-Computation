...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        output = pooled_output.reshape(input_ids.size(0),-1).contiguous()
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                if self.config.num_labels == 3:
                    loss_fct = CrossEntropyLoss(weight = torch.FloatTensor([0.3,0.2,0.5]).cuda())
                else:
                    loss_fct = CrossEntropyLoss(weight = torch.FloatTensor([0.45,0.55]).cuda())
